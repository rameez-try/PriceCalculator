...
//				console.log("datafolder : "+datafolder)
      ...
      $('.GridSelect').change(function ()
			...
			function getGrid(GridTab)
			{
				...
				promise.done(function(data)
				{
					...
					$.each(data, function(i, item) {
						...
						window.pack = item[5];
						window.basic = item[6];
						window.cinema = item[7];
						window.SandC = item[8];
						window.sport = item[9];
						populateGrid(basic, sport, cinema, SandC);
						j ++;
						});
		...
		function populateGrid(basic, sport, cinema, SandC) 
		{
			var tableRow = 0;
			var tblName = "";
			var GridType = 1;
			...
			row+= "<td class='"+grid+"cell C"+cell+" highlight'><span class='price "+curr+"'>" + basic; +"</span><span class='originalprice hidden'></span></td>"
			row+= "<td class='"+grid+"cell C"+cell+" highlight'><span class='price "+curr+"'>" + cinema; +"</span><span class='originalprice hidden'></span></td>"
			row+= "<td class='"+grid+"cell C"+cell+" highlight'><span class='price "+curr+"'>" + sport; +"</span><span class='originalprice hidden'></span></td>"
			row+= "<td class='"+grid+"cell C"+cell+" highlight'><span class='price "+curr+"'>" + SandC; +"</span><span class='originalprice hidden'></span></td>"
		...
		{
			console.log("in the apply discount function");
			var discount = DiscountAmount/100;
		
			if(option == "Basic (ONLY)") 
			{
				var newbasic = (basic-(basic*discount)).toFixed(2);
				console.log("basic");

			}
				
			if(option == "Sky Cinema (ONLY)") 
			{
				var newcinema = (cinema-(cinema*discount)).toFixed(2);
				console.log("Cinema");
			}
				
			if(option == "Sky Sports (ONLY)") 
			{
				var newsports = (sport-(sport*discount)).toFixed(2);
				console.log("Sports");
			}
				
			if(option == "Sky Sports and Cinema (ONLY)") 
			{
				var newSandC = (SandC-(SandC*discount)).toFixed(2);
				console.log("SandC");
			}
			...
			populateGrid(newbasic, newsports, newcinema, newSandC);
		}
		...
		function applyMoneyDiscount(DiscountAmount, option) 
		{
			...
			if(option == "Basic (ONLY)")
			{
				var newbasic = basic - discount;
			}
	
			if(option == "Sky Cinema (ONLY)") 
			{
				var newcinema = cinema - discount;
			}
	
			if(option == "Sky Sports (ONLY)")
			{
				var newsports = sport - discount;
			}
	
			if(option == "Sky Sports and Cinema (ONLY)")
			{
				var newSandC = SandC - discount;
			}
		...
		function getDiscount(table, tableNm, NumRow) 
		{
			...
			var option = table.rows[1].cells[3].children[0].value;
			if (DiscountType == 1) 
			{
