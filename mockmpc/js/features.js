...
basket.require
	...
	/*-- Functions  --------------------------------------------------------------------------------------------*/
			...
			function getUrlPriceRange()
			{
				...
				//the chooseFolder() function works now, now I need to set it so that it sends specific parameters based on which option the user selects
				if (url.indexOf('?') > -1)
				{
					//chooseFolder(url, p);
			...
			function getGrid(GridTab)
			{
				...
				jQuery.ajax({
					...
					success: function (data) {
						...
						myTable+="</table>";
						document.getElementById('priceTable').innerHTML = myTable;
/*
.butt:hover {
...
*/
			...
			function addDiscount(table,tableNm,NumRow)
			{
				...
				var DiscountType = table.rows[1].cells[2].children[0].value;
					...
					if (DiscountType==1)
					{
						DiscountTypeSymb=DiscountAmount+"%";
						DiscountSum=DiscountAmount/100;
						DiscountsumSymb="1";
					}
					else
					{
						DiscountTypeSymb="<span class='"+curr+"Tab'></span>"+DiscountAmount;
						DiscountSum=DiscountAmount;
						DiscountsumSymb="2";
					}

					var DiscountModifiedTxt=""

					if((tableNm=="BoxType_Discounts") && ($("#PackageType option:selected").val()!="CX"))
					{
						DiscountModifiedTxt=$("#PackageType option:selected").text()
					}

					//add rows, getting content from 1st row cells
						row.insertCell(0).innerHTML = DiscountTypeSymb+" off "+DiscountModifiedTxt+", for "+DiscountFor;
						row.insertCell(1).innerHTML = DiscountSum;
						row.insertCell(2).innerHTML = DiscountsumSymb;

					if	(tableNm=="BoxType_Discounts")
					{// if entertainment grid discount

						//add rows, getting content from 1st row cells
						row.insertCell(3).innerHTML = $("#PackageType option:selected").val();

						//insert delete button in last cell of new row
						row.insertCell(4).innerHTML = "<p class='editRow deleteRow'><i title='minus' class='fa icon-minus tableIcon'></i></p>"
						$("#"+tableNm+" tbody tr:not(:first) td:nth-child(4)").addClass("whiteout");
					}
					else
					{//if any other type of discount
						//insert delete button in last cell of new row
						row.insertCell(3).innerHTML = "<p class='editRow deleteRow'><i title='minus' class='fa icon-minus tableIcon'></i></p>"
					}

					//hide the calculation values in the Discount table results
					$("#"+tableNm+" tbody tr:not(:first) td:nth-child(2)").addClass("whiteout");
					$("#"+tableNm+" tbody tr:not(:first) td:nth-child(3)").addClass("whiteout");

					console.log("Add Discount to discount table "+tableNm);

					//Apply discount to price grid
					UpdateGrid(tableNm);
			...
			$(document.body).on("click touchstart", ".editRow", function ()
			{
				...
				{
					//if adding a row
					addDiscount(table,tableNm,NumRow)
				}
				else
				{
							...
							if (symb==1)
							{
								...
								if (ColumnMod=="CX")
								{
									  //if Whole is selected apply the change to the whole grid price
										...
										NewGridPrice=parseFloat(GridPrice-wholeDiscount).toFixed(2)
								}
								else
